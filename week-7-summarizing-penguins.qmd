...
```{r create-ratio}
#| label: creating-bill-ratio

#create bill ratio variable
penguins <- penguins |> 
...
```{r boxplot}
#| label: bill-ratio-boxplot

#boxplot to visualize bill_ratio by species
ggplot(data = penguins) +
  geom_boxplot(aes(x = bill_ratio,
                   y = species,
                   fill = species)) +
  labs(x = "Ratio of Bill Length to Bill Depth",
       y = "Penguin Species") +
  guides(fill = "none") +
  theme_bw() +
...
```{r new-columns}
#| label: new-columns-conversions

#create new columns for converted variables
penguins <- penguins |>
  mutate(bill_ratio = bill_length_mm / bill_depth_mm,
...
#reorder species and recreate boxplot
penguins |> 
  mutate(species = 
...
```{r mean-sd-mass-kg}
#| label: summary-stats-mass-kg
#Calculate summary statistics for body mass (kg) by species and sex
penguins |> 
  mutate(body_mass_kg = body_mass_g / 1000) |> 
  group_by(species, sex) |> 
  summarize(mass_mean_kg = mean(body_mass_kg, na.rm = TRUE),
            mass_sd_kg = sd(body_mass_kg, na.rm = TRUE))
```
...
```{r}
#| label: min-max-flipper-length
#table for the minimum and maximum flipper length for Chinstrap
penguins |> 
  filter(species == "Chinstrap") |> 
  group_by(island) |> 
  mutate(flip_min = min(flipper_length_mm),
...
```{r}
#| label: bill-ratio-table
#summary table for mean bill ratio by species
penguins |> 
  mutate(bill_ratio = bill_length_mm/bill_depth_mm) |> 
  select(species, bill_ratio) |> 
  group_by(species) |> 
  filter(complete.cases(bill_ratio)) |> #https://stackoverflow.com/a/75847863 
  summarize(bill_ratio_mean = mean(bill_ratio)) 
